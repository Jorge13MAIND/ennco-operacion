...
**Estado: publicación técnica en producción autorizada por el usuario el 10 de septiembre de 2026; aceptación operativa de ENNCO pendiente.** La solicitud explícita autoriza publicar en `https://ennco-operacion.vercel.app/operacion`, aplicar sólo M055 a la base dedicada y configurar la clave privada requerida. Esta autorización no acredita que el despliegue ya haya terminado: el acta de publicación debe registrar SHA, migración, despliegue y comprobaciones efectivas. Hay un recorrido técnico y administrativo implementado para los tres segmentos y pruebas sintéticas de sus controles. Faltan OAuth/Drive comprobados, datos operativos aprobados, pruebas con los recibos reales de Paco, validación contractual y aceptación en el entorno conectado. No se presenta como recuperación exacta de todas las fórmulas del Excel.

Este documento complementa [la especificación del motor](projects-engineering.md), [la evaluación de OCR](projects-ocr-evaluation.md) y [la plantilla contractual](projects-contract-template.md). La autorización de publicación procede de la solicitud posterior del usuario, no del permiso previo para construir. Se conservan autenticación, configuración de envíos y estados de revisión. Drive continuará pendiente visible hasta disponer de consentimiento OAuth verificable de `contacto@ennco.com.mx`; los documentos conservan almacenamiento privado intermedio.
...
Criterio de aceptación operativa: tres recorridos aceptados por Paco y responsables, ausencia de defectos críticos abiertos, permisos/OAuth/Drive comprobados y parámetros aprobados. El usuario autorizó la publicación técnica con estos pendientes conservados; no equivale a completar el acta de aceptación. La disponibilidad productiva requiere además aplicar M055 al destino autorizado, aprobar las comprobaciones técnicas del SHA y verificar el despliegue. Mientras falten las validaciones de ENNCO, el paquete continúa en validación operativa. Si una tarifa, configuración o selección no está respaldada, debe mantenerse identificada como fuera de cobertura final; no habilitar una aprobación genérica para ocultar el pendiente.
...
El preview de revisión está disponible en https://ennco-operacion-git-grant-proyect-6ce00c-jorge13mainds-projects.vercel.app/operacion/proyectos. La verificación externa del primer candidato confirmó tres ejemplos, sólo lectura, PDF válido, respuesta 409 a escrituras y cero incidencias de axe/JavaScript (`evidence/projects/preview-verification.json`). Se ajustó posteriormente el tamaño de los importes largos y se incorporó el job dedicado `projects` al CI; seis E2E afectados, lint, tipos y build volvieron a pasar.
