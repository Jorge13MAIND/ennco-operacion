...
export const directLaneMailboxHealthSchema = z.object({
  ...
  ramp_mode: z.enum(["AUTO", "FIXED", "SCHEDULE"]),
  ramp_schedule: z.array(z.number().int()).nullable().optional(),
  ramp_anchor_at: z.string().nullable().optional(),
  ramp_week: z.number().int().nullable().optional(),
  new_today: z.number().int().optional(),
  new_cap: z.number().int().optional(),
