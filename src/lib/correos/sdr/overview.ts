...
export type SdrScreen = { available: boolean; live: boolean; cases: SdrCaseView[]; status: z.infer<typeof statusSchema> | null };
export async function loadSdrScreen(access: OperationsAccessContext): Promise<SdrScreen> {
  if (access.evidenceClass !== "live" || !access.organizationId) return { available: true, live: false, cases: [], status: null };
  ...
  if (cases.error || status.error || !parsedCases.success || !parsedStatus.success) return { available: false, live: true, cases: [], status: null };
  return { available: true, live: true, cases: parsedCases.data, status: parsedStatus.data };
}
