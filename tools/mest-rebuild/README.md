...
Sin dependencias fuera de la biblioteca estándar salvo `vba_patch.py` y `vba_clean.py` (pyOpenVBA) y `make_images.py` (Pillow), que corren en el venv `~/.local/opt/pyenv-mest`.
...
- `vba_patch.py` · reescribe los módulos VBA tóxicos o desfasados; `vba_clean.py` deja el proyecto solo con código fuente (sin p-code viejo, `_VBA_PROJECT` = `CC 61 FF FF 00 00 00`, sin candado) para que Excel lo recompile al abrir: sin este paso Excel descarta el proyecto y dice "No se puede ejecutar la macro"; `vba_check.py` los lee sin Office y busca cadenas prohibidas.
