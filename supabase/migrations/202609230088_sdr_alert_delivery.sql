begin;
create table app.email_sdr_alert_dispatch (
  organization_id uuid not null references public.organizations(id),
  case_id uuid not null references public.email_sdr_cases(id),
  stage text not null check(stage in ('PRIMARY','BACKUP','NO_BACKUP','OVERDUE')),
  attempts integer not null default 0 check(attempts between 0 and 100),
  last_attempt_at timestamptz,
  provider_accepted_at timestamptz,
  last_error text,
  primary key(organization_id,case_id,stage)
);
revoke all on app.email_sdr_alert_dispatch from public,anon,authenticated,service_role;

create function app.email_sdr_alert_work(target_org uuid)
...
begin
  ...
  if coalesce(oldest_seconds,0)>=7200 then
    update public.campaigns ca set new_contacts_paused=true,updated_at=clock_timestamp()
      ...
      and ca.id=any((select campaign_ids from app.email_sdr_settings where organization_id=target_org));
...
revoke all on function app.email_sdr_alert_work(uuid) from public,anon,authenticated,service_role;

create function app.email_sdr_alert_settle(target_org uuid,target_case uuid,target_stage text,target_accepted boolean)
returns jsonb language plpgsql security definer set search_path=public,app,pg_temp as $$
declare changed integer;
begin
  if target_accepted is null then raise exception 'SDR_ALERT_RESULT_INVALID'; end if;
  update app.email_sdr_alert_dispatch set provider_accepted_at=case when target_accepted then clock_timestamp() else null end,
    last_error=case when target_accepted then null else 'ALERT_PROVIDER_UNAVAILABLE' end
    where organization_id=target_org and case_id=target_case and stage=target_stage
      and provider_accepted_at is null and last_attempt_at>clock_timestamp()-interval '4 minutes';
  get diagnostics changed=row_count;
  if changed<>1 then raise exception 'SDR_ALERT_RESULT_NOT_CLAIMED'; end if;
  return jsonb_build_object('status',case when target_accepted then 'PROVIDER_ACCEPTED' else 'PROVIDER_UNAVAILABLE' end);
end $$;
revoke all on function app.email_sdr_alert_settle(uuid,uuid,text,boolean) from public,anon,authenticated,service_role;
commit;
