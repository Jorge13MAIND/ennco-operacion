begin;
-- Auto-enrollment skips unready contacts rather than failing an entire batch.
...
declare campaign_record public.campaigns%rowtype;
  ...
  skipped_unverified integer := 0; skipped_not_ready integer := 0; skipped_enrolled integer := 0; by_variant jsonb := '{}'::jsonb;
...
begin
  ...
  loop
    ...
    if not app.email_contact_is_ready(target_organization_id,candidate.contact_id) then
      skipped_not_ready := skipped_not_ready+1; continue;
    end if;
  ...
  values (target_organization_id,actor,audit_action,'campaigns',target_campaign_id,
    jsonb_build_object('enrolled',enrolled,'skipped_suppressed',skipped_suppressed,'skipped_unverified',skipped_unverified,
      'skipped_enrolled',skipped_enrolled,'skipped_not_ready',skipped_not_ready,'by_variant',by_variant,'by_mailbox',by_mailbox));
  return jsonb_build_object('status','ENROLLED','campaign_id',target_campaign_id,'enrolled',enrolled,
    'skipped_suppressed',skipped_suppressed,'skipped_unverified',skipped_unverified,'skipped_enrolled',skipped_enrolled,'skipped_not_ready',skipped_not_ready,
