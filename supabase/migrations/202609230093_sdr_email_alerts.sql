begin;
alter table app.email_sdr_alert_dispatch add column uncertain_at timestamptz;
...
create or replace function app.email_sdr_alert_settle(target_org uuid,target_case uuid,target_stage text,target_accepted boolean)
returns jsonb language plpgsql security definer set search_path=public,app,pg_temp as $$
declare changed integer;
begin
  update app.email_sdr_alert_dispatch set provider_accepted_at=case when target_accepted is true then clock_timestamp() else null end,
    uncertain_at=case when target_accepted is null then clock_timestamp() else null end,
    last_error=case when target_accepted is true then null when target_accepted is null then 'ALERT_PROVIDER_AMBIGUOUS'
      else 'ALERT_PROVIDER_UNAVAILABLE' end
    where organization_id=target_org and case_id=target_case and stage=target_stage
      and provider_accepted_at is null and uncertain_at is null
      and last_attempt_at>clock_timestamp()-interval '4 minutes';
  get diagnostics changed=row_count;
  if changed<>1 then raise exception 'SDR_ALERT_RESULT_NOT_CLAIMED'; end if;
  return jsonb_build_object('status',case when target_accepted is true then 'PROVIDER_ACCEPTED'
    when target_accepted is null then 'PROVIDER_AMBIGUOUS' else 'PROVIDER_UNAVAILABLE' end);
end $$;
commit;
