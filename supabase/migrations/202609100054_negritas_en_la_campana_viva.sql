-- M054: negritas en la campana viva.
--
-- Grant, 10-sep: resaltar en negritas solo las palabras mas importantes. El
-- copy marca con **asi** (maximo dos frases por correo: el beneficio y la
-- peticion; en el toque 8, la palabra "baja"). El motor manda el texto plano
-- sin marcadores y un HTML minimo con <strong>; el texto de los correos no
-- cambia en una sola palabra.
--
-- Igual que la 051 y la 053: lleva asunto y cuerpo de los 32 toques a la
-- campana ya aprobada, anclada al sha vigente (o al nuevo, para reaplicar),
-- actualiza los sha por variante y el manifiesto y lo anota en audit_log. En
-- una base nueva (CI) no encuentra nada y no toca una fila.

do $$
declare
  sha_secuencia_anterior constant text := 'b1ec779fd7f29391ff89457639b748e99ae33adea7c7f9da0f8bc3cda8cdb7f6';
  sha_secuencia_nueva    constant text := '6505f46fca41be06bf0fbdef4c076c00fe0a322d716a634959f0854fb3355a47';
  sha_fuente_nueva       constant text := '230edbd05b04617870841e88b31d5fed0faf17afa88957bc47d8ae520aa9f5cd';
  sha_por_variante constant jsonb := jsonb_build_object(
    '1', '44690686edbaf2f82797665c86765a8c4441df63b80589b9ab3e3167c07ee433',
    '2', '24f58923a8bd79f63d64c0eb010789487f9e1a114eab9ea135c51960083642af',
    '3', '05b171a47142013b1a223f3df86ac1c4987b0bc6361031db8a37ec00d7b5c8d4',
    '4', 'dc2e139fd0950b9405faf58203a6b22b042c51c24510914dcea857680d092f82'
  );
  campana record;
  manifiesto jsonb;
...
Francisco')
    ...
    if manifiesto is distinct from campana.manifest_json then
      ...
      values (
        campana.organization_id, null, 'DIRECT_LANE_CAMPAIGN_COPY_BOLD', 'campaigns', campana.id,
        ...
        jsonb_build_object(
          ...
          'authorized_by', 'Grant Keegan, teckel_admin, 2026-09-10',
          'reason', 'Negritas en el beneficio y la peticion; el texto no cambia'));
    end if;
  end loop;
end $$;
