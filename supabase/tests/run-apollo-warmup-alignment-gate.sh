...
DB_GATE_DIR="$(mktemp -d /tmp/ennco-apollo-warmup-gate.XXXXXX)"
DB_GATE_PORT="${ENNCO_APOLLO_WARMUP_GATE_PORT:-55464}"
DB_GATE_NAME="ennco_apollo_warmup_gate"

cleanup() {
  pg_ctl -D "$DB_GATE_DIR/data" stop -m fast >/dev/null 2>&1 || true
  case "$DB_GATE_DIR" in
    /tmp/ennco-apollo-warmup-gate.*) find "$DB_GATE_DIR" -depth -delete ;;
...
psql -X -v ON_ERROR_STOP=1 -h "$DB_GATE_DIR" -p "$DB_GATE_PORT" -d "$DB_GATE_NAME" \
  -f "$REPO_ROOT/supabase/tests/008_first_send_release_gate.sql" >/dev/null
...
psql -X -v ON_ERROR_STOP=1 -h "$DB_GATE_DIR" -p "$DB_GATE_PORT" -d "$DB_GATE_NAME" \
  -f "$REPO_ROOT/supabase/tests/023_apollo_warmup_alignment_rollback_gate.sql"
...
git -C "$REPO_ROOT" diff --check -- \
  supabase/migrations/202608140023_apollo_warmup_alignment.sql \
  supabase/rollbacks/202608140023_apollo_warmup_alignment.down.sql \
  supabase/tests/023_apollo_warmup_alignment_gate.sql \
  supabase/tests/023_apollo_warmup_alignment_rollback_gate.sql \
  supabase/tests/run-apollo-warmup-alignment-gate.sh
printf 'APOLLO_WARMUP_ALIGNMENT_DIFF_CHECK_PASS\n'
