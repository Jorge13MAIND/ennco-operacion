...
GATE_DIR="$(mktemp -d /tmp/ennco-email-recovery-gate.XXXXXX)"
GATE_PORT="${ENNCO_RECOVERY_GATE_PORT:-55568}"
GATE_DB="ennco_direct_lane_gate"

cleanup() {
  pg_ctl -D "$GATE_DIR/data" stop -m fast >/dev/null 2>&1 || true
  case "$GATE_DIR" in
    /tmp/ennco-email-recovery-gate.*) find "$GATE_DIR" -depth -delete ;;
