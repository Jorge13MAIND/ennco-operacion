...
DB_GATE_DIR="$(mktemp -d /tmp/ennco-prequote-gate.XXXXXX)"
DB_GATE_PORT="${ENNCO_PREQUOTE_GATE_PORT:-55436}"
DB_GATE_NAME="ennco_prequote_gate"

cleanup() {
  pg_ctl -D "$DB_GATE_DIR/data" stop -m fast >/dev/null 2>&1 || true
  case "$DB_GATE_DIR" in
    /tmp/ennco-prequote-gate.*) find "$DB_GATE_DIR" -depth -delete ;;
...
psql -X -v ON_ERROR_STOP=1 -h "$DB_GATE_DIR" -p "$DB_GATE_PORT" -d "$DB_GATE_NAME" \
  -f "$REPO_ROOT/supabase/tests/004_public_prequote_capture_gate.sql"

psql -X -v ON_ERROR_STOP=1 -h "$DB_GATE_DIR" -p "$DB_GATE_PORT" -d "$DB_GATE_NAME" \
  -f "$REPO_ROOT/supabase/rollbacks/202608110004_public_prequote_capture.down.sql" >/dev/null
psql -X -v ON_ERROR_STOP=1 -h "$DB_GATE_DIR" -p "$DB_GATE_PORT" -d "$DB_GATE_NAME" \
  -f "$REPO_ROOT/supabase/tests/004_public_prequote_capture_rollback_gate.sql"

psql -X -v ON_ERROR_STOP=1 -h "$DB_GATE_DIR" -p "$DB_GATE_PORT" -d "$DB_GATE_NAME" \
  -f "$REPO_ROOT/supabase/migrations/202608110004_public_prequote_capture.sql" >/dev/null
printf 'PUBLIC_PREQUOTE_CAPTURE_REAPPLY_PASS\n'
