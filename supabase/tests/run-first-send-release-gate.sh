...
DB_GATE_DIR="$(mktemp -d /tmp/ennco-first-send-release-gate.XXXXXX)"
DB_GATE_PORT="${ENNCO_FIRST_SEND_GATE_PORT:-55440}"
DB_GATE_NAME="ennco_first_send_release_gate"

cleanup() {
  pg_ctl -D "$DB_GATE_DIR/data" stop -m fast >/dev/null 2>&1 || true
  case "$DB_GATE_DIR" in
    /tmp/ennco-first-send-release-gate.*) find "$DB_GATE_DIR" -depth -delete ;;
...
psql -X -v ON_ERROR_STOP=1 -h "$DB_GATE_DIR" -p "$DB_GATE_PORT" -d "$DB_GATE_NAME" \
  -f "$REPO_ROOT/supabase/tests/008_first_send_release_gate.sql"

psql -X -v ON_ERROR_STOP=1 -h "$DB_GATE_DIR" -p "$DB_GATE_PORT" -d "$DB_GATE_NAME" \
  -f "$REPO_ROOT/supabase/rollbacks/202608110008_first_send_release.down.sql" >/dev/null
psql -X -v ON_ERROR_STOP=1 -h "$DB_GATE_DIR" -p "$DB_GATE_PORT" -d "$DB_GATE_NAME" \
  -f "$REPO_ROOT/supabase/tests/008_first_send_release_rollback_gate.sql"

psql -X -v ON_ERROR_STOP=1 -h "$DB_GATE_DIR" -p "$DB_GATE_PORT" -d "$DB_GATE_NAME" \
  -f "$REPO_ROOT/supabase/migrations/202608110008_first_send_release.sql" >/dev/null
printf 'FIRST_SEND_RELEASE_REAPPLY_PASS\n'
