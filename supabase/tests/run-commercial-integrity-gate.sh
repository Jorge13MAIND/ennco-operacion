...
DB_GATE_DIR="$(mktemp -d /tmp/ennco-commercial-integrity-gate.XXXXXX)"
DB_GATE_PORT="${ENNCO_COMMERCIAL_INTEGRITY_GATE_PORT:-55447}"
DB_GATE_NAME="ennco_commercial_integrity_gate"

cleanup() {
  pg_ctl -D "$DB_GATE_DIR/data" stop -m fast >/dev/null 2>&1 || true
  case "$DB_GATE_DIR" in
    /tmp/ennco-commercial-integrity-gate.*) find "$DB_GATE_DIR" -depth -delete ;;
...
psql -X -v ON_ERROR_STOP=1 -h "$DB_GATE_DIR" -p "$DB_GATE_PORT" -d "$DB_GATE_NAME" \
  -f "$REPO_ROOT/supabase/tests/014_commercial_integrity_gate.sql"

psql -X -v ON_ERROR_STOP=1 -h "$DB_GATE_DIR" -p "$DB_GATE_PORT" -d "$DB_GATE_NAME" \
  -f "$REPO_ROOT/supabase/rollbacks/202608120014_commercial_integrity.down.sql" >/dev/null
psql -X -v ON_ERROR_STOP=1 -h "$DB_GATE_DIR" -p "$DB_GATE_PORT" -d "$DB_GATE_NAME" \
  -f "$REPO_ROOT/supabase/tests/014_commercial_integrity_rollback_gate.sql"

psql -X -v ON_ERROR_STOP=1 -h "$DB_GATE_DIR" -p "$DB_GATE_PORT" -d "$DB_GATE_NAME" \
  -f "$REPO_ROOT/supabase/migrations/202608120014_commercial_integrity.sql" >/dev/null
printf 'COMMERCIAL_INTEGRITY_REAPPLY_PASS\n'
