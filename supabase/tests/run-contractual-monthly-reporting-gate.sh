...
DB_GATE_DIR="$(mktemp -d /tmp/ennco-contractual-reporting-gate.XXXXXX)"
DB_GATE_PORT="${ENNCO_CONTRACTUAL_REPORTING_GATE_PORT:-55442}"
DB_GATE_NAME="ennco_contractual_reporting_gate"

cleanup() {
  pg_ctl -D "$DB_GATE_DIR/data" stop -m fast >/dev/null 2>&1 || true
  case "$DB_GATE_DIR" in
    /tmp/ennco-contractual-reporting-gate.*) find "$DB_GATE_DIR" -depth -delete ;;
...
psql -X -v ON_ERROR_STOP=1 -h "$DB_GATE_DIR" -p "$DB_GATE_PORT" -d "$DB_GATE_NAME" \
  -f "$REPO_ROOT/supabase/tests/010_contractual_monthly_reporting_gate.sql"

psql -X -v ON_ERROR_STOP=1 -h "$DB_GATE_DIR" -p "$DB_GATE_PORT" -d "$DB_GATE_NAME" \
  -f "$REPO_ROOT/supabase/rollbacks/202608120010_contractual_monthly_reporting.down.sql" >/dev/null
psql -X -v ON_ERROR_STOP=1 -h "$DB_GATE_DIR" -p "$DB_GATE_PORT" -d "$DB_GATE_NAME" \
  -f "$REPO_ROOT/supabase/tests/010_contractual_monthly_reporting_rollback_gate.sql"

psql -X -v ON_ERROR_STOP=1 -h "$DB_GATE_DIR" -p "$DB_GATE_PORT" -d "$DB_GATE_NAME" \
  -f "$REPO_ROOT/supabase/migrations/202608120010_contractual_monthly_reporting.sql" >/dev/null
printf 'CONTRACTUAL_MONTHLY_REPORTING_REAPPLY_PASS\n'
