...
DB_GATE_DIR="$(mktemp -d /tmp/ennco-one-click-unsubscribe-gate.XXXXXX)"
DB_GATE_PORT="${ENNCO_ONE_CLICK_UNSUBSCRIBE_GATE_PORT:-55445}"
DB_GATE_NAME="ennco_one_click_unsubscribe_gate"

cleanup() {
  pg_ctl -D "$DB_GATE_DIR/data" stop -m fast >/dev/null 2>&1 || true
  case "$DB_GATE_DIR" in
    /tmp/ennco-one-click-unsubscribe-gate.*) find "$DB_GATE_DIR" -depth -delete ;;
...
psql -X -v ON_ERROR_STOP=1 -h "$DB_GATE_DIR" -p "$DB_GATE_PORT" -d "$DB_GATE_NAME" \
  -f "$REPO_ROOT/supabase/tests/013_one_click_unsubscribe_gate.sql"
psql -X -v ON_ERROR_STOP=1 -h "$DB_GATE_DIR" -p "$DB_GATE_PORT" -d "$DB_GATE_NAME" \
  -f "$REPO_ROOT/supabase/rollbacks/202608120013_one_click_unsubscribe.down.sql" >/dev/null
psql -X -v ON_ERROR_STOP=1 -h "$DB_GATE_DIR" -p "$DB_GATE_PORT" -d "$DB_GATE_NAME" \
  -f "$REPO_ROOT/supabase/tests/013_one_click_unsubscribe_rollback_gate.sql"
psql -X -v ON_ERROR_STOP=1 -h "$DB_GATE_DIR" -p "$DB_GATE_PORT" -d "$DB_GATE_NAME" \
  -f "$REPO_ROOT/supabase/migrations/202608120013_one_click_unsubscribe.sql" >/dev/null
printf 'ONE_CLICK_UNSUBSCRIBE_REAPPLY_PASS\n'
